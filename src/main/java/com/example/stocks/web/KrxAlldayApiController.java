...
import com.example.stocks.bot.KrxAlldayScannerService;
...
@RestController
@RequestMapping("/api/krx-allday")
public class KrxAlldayApiController {

    private final KrxAlldayScannerService scannerService;
    private final KrxAlldayConfigRepository configRepo;
    private final BotConfigRepository botConfigRepo;

    public KrxAlldayApiController(KrxAlldayScannerService scannerService, KrxAlldayConfigRepository configRepo, BotConfigRepository botConfigRepo) {
        this.scannerService = scannerService; this.configRepo = configRepo; this.botConfigRepo = botConfigRepo;
    }

    @PostMapping("/start") public ResponseEntity<Map<String, Object>> start() { KrxAlldayConfigEntity cfg = configRepo.loadOrCreate(); cfg.setEnabled(true); configRepo.save(cfg); scannerService.start(); return ResponseEntity.ok(buildStatus()); }
    @PostMapping("/stop") public ResponseEntity<Map<String, Object>> stop() { KrxAlldayConfigEntity cfg = configRepo.loadOrCreate(); cfg.setEnabled(false); configRepo.save(cfg); scannerService.stop(); return ResponseEntity.ok(buildStatus()); }
    ...
    public ResponseEntity<Map<String, Object>> updateConfig(@RequestBody Map<String, Object> body) {
        ...
        if (body.containsKey("entryEndHour")) cfg.setEntryEndHour(toInt(body.get("entryEndHour"), 14));
    ...
    private Map<String, Object> configToMap(KrxAlldayConfigEntity cfg) {
        Map<String, Object> m = new LinkedHashMap<String, Object>();
        ...
        m.put("globalCapitalKrw", (!bcs.isEmpty() && bcs.get(0).getCapitalKrw() != null) ? bcs.get(0).getCapitalKrw() : BigDecimal.valueOf(500000));
