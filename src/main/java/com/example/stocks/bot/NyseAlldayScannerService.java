...
/**
 * NYSE AllDay High-Confidence Breakout Scanner.
 * ET timezone (auto DST via ZoneId.of("America/New_York")).
 * Entry 11:30-15:00 ET, Session end 15:45 ET.
 * SPY filter, high confidence breakout, Quick TP.
 * scanner_source = "NYSE_ALLDAY"
 */
@Service
public class NyseAlldayScannerService {

    private static final Logger log = LoggerFactory.getLogger(NyseAlldayScannerService.class);
    private static final ZoneId ET = ZoneId.of("America/New_York");
    private static final String SCANNER_SOURCE = "NYSE_ALLDAY";
    private static final String ENTRY_STRATEGY = "NYSE_HIGH_CONFIDENCE";
    ...
    private final ExecutorService parallelExecutor = Executors.newFixedThreadPool(
            ...
            new ThreadFactory() {
                ...
                public Thread newThread(Runnable r) {
                    Thread t = new Thread(r, "nyse-allday-parallel-" + counter.incrementAndGet());
                    t.setDaemon(true);
                    return t;
                }
            }
    ...
    public NyseAlldayScannerService(NyseAlldayConfigRepository configRepo,
    ...
    public boolean start() {
        if (!running.compareAndSet(false, true)) {
            log.info("[NyseAllday] already running");
            return false;
        }
        log.info("[NyseAllday] starting...");
        statusText = "RUNNING";
        scheduler = Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {
            @Override public Thread newThread(Runnable r) {
                Thread t = new Thread(r, "nyse-allday-scanner"); t.setDaemon(true); return t;
            }
        });
        scheduleTick();
        startQuickTpTicker();
    ...
    public boolean stop() {
        if (!running.compareAndSet(true, false)) {
            log.info("[NyseAllday] already stopped");
            return false;
        }
        log.info("[NyseAllday] stopping...");
        statusText = "STOPPED";
        stopQuickTpTicker();
    ...
    private void scheduleTick() {
        ...
        try {
            ...
            log.error("[NyseAllday] schedule failed", e);
        }
    }

    private void tickWrapper() {
        try { tick(); } catch (Exception e) { log.error("[NyseAllday] tick error", e); }
    ...
    private void tick() {
                    ...
                    @Override public CandleFetchResult call() {
                        try {
                            List<StockCandle> candles = candleService.getMinuteCandles(
                                    pe.getSymbol(), MarketType.NYSE, sellCandleUnit, 80, null);
                ...
                try {
                    ...
                    if (nowMinOfDay >= sessionEnd) {
                        executeSell(pe, candles.get(candles.size() - 1),
                                Signal.of(SignalAction.SELL, null, "SESSION_END: NYSE allday session closing"), cfg);
        ...
        if (canEnter) {
            ...
            for (final String symbol : entryCandidates) {
                buyFutures.put(symbol, parallelExecutor.submit(new Callable<CandleFetchResult>() {
                    @Override public CandleFetchResult call() {
                        try {
                            List<StockCandle> candles = candleService.getMinuteCandles(
                                    symbol, MarketType.NYSE, buyCandleUnit, 80, null);
            ...
            log.info("[NyseAllday] tick done mode={} symbols={} attempts={} signals={} entries={} positions={}",
    ...
    private void executeBuy(String symbol, StockCandle candle, Signal signal,
        ...
        log.info("[NyseAllday] BUY {} mode={} price={} qty={} conf={}", symbol, cfg.getMode(), fillPrice, qty, signal.confidence);
    }
    ...
    private void executeSell(PositionEntity pe, StockCandle candle, Signal signal,
        ...
        log.info("[NyseAllday] SELL {} price={} pnl={} roi={}%", pe.getSymbol(), fillPrice,
    ...
    private void startQuickTpTicker() {
        ...
        tickerExec = Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {
            @Override public Thread newThread(Runnable r) {
                Thread t = new Thread(r, "nyse-allday-quick-tp"); t.setDaemon(true); return t;
            }
        });
        tickerFuture = tickerExec.scheduleAtFixedRate(new Runnable() {
            @Override public void run() {
                try { tickQuickTp(); } catch (Exception e) { log.error("[NyseAllday] Quick TP error", e); }
            }
        ...
        for (PositionEntity pe : positionRepo.findAll()) {
            ...
            try {
                ...
                try {
                    ...
                    log.debug("[NyseAllday] Failed to lookup confidence for {}: {}", pe.getSymbol(), e.getMessage());
                }
                ...
                log.error("[NyseAllday] Quick TP failed for {}", pe.getSymbol(), e);
            }
        }
    }
    ...
    private BigDecimal calcOrderSize(NyseAlldayConfigEntity cfg) {
    ...
    private boolean checkIndexFilter(String indexSymbol, int candleUnit, int emaPeriod) {
        try {
            ...
            log.error("[NyseAllday] Index filter check failed", e);
            return true;
        }
    }
    ...
    private List<String> getTopSymbolsByVolume(int topN, Set<String> excludeSymbols, MarketType marketType) {
        try {
            ...
            log.info("[NyseAllday] Volume ranking: {} symbols selected (excluded {})",
                    filtered.size(), excludeSymbols.size());
            return filtered;
        } catch (Exception e) {
            log.error("[NyseAllday] Failed to fetch volume ranking", e);
